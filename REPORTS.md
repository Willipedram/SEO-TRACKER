...
Movement classifications use a database window query over the bounded date range, while Search Console aggregation and pagination happen in SQL. Schema 14 adds the audited date-window and website/date indexes used by these reports. Production deployments must use MySQL 8+ because movement reports use `ROW_NUMBER()`.
