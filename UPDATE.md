...
`config/version.php` is the source authority for application version `1.3.0` and
target schema version `11`. Persistent `app_installations.source_version` and
...
Phase 11 schema version 8 adds only idempotent Rank Dashboard read indexes. It does
not modify or synthesize rank history.
...
Application version 1.1.0 targets schema version 9. Migration
`2026_08_23_080000_search_console_foundation` adds the optional Search Console
connection foundation and a disabled module lifecycle row without touching Core,
website, keyword, or ranking data.

Application version 1.2.0 targets schema version 10. Migration
`2026_08_23_090000_search_console_oauth` adds property candidates and connection-to-
website context tables and updates only the optional module version. Existing Search
Console connections and all Core/ranking data are preserved.

Application version 1.3.0 targets schema version 11. Migration
`2026_08_24_000000_search_console_sync` adds only optional-module sync queue, lifecycle
log, and dimension-metric tables plus indexes, then advances the Search Console module
to 1.2.0. Existing OAuth connections, Core data, and rank history are preserved.
