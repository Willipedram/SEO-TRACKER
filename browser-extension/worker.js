  ...
  (async () => {
    ...
    try {
      ...
      for (let page=0; page<10; page++) {
        if(page>0){const next=new URL(first);next.searchParams.set('start',String(page*10));await notify(dashboardTabId,{type:'SEO_RANK_PROGRESS',id:job.id,progress:page*10,message:`در حال رفتن به صفحه ${page+1} Google…`});await navigate(tabId,next.toString());}else await loaded(tabId);
