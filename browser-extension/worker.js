...
const applyDeviceProfile = async (tabId,device) => {
  ...
  await chrome.debugger.sendCommand({tabId},'Network.setUserAgentOverride',{userAgent,platform:mobile?'Android':'Windows'});
  await chrome.debugger.sendCommand({tabId},'Emulation.setDeviceMetricsOverride',{width:mobile?412:1366,height:mobile?915:768,deviceScaleFactor:mobile?2.625:1,mobile});
  return {name:mobile?'mobile-android':'desktop-windows',userAgent};
};
...
chrome.runtime.onMessage.addListener((job, sender, reply) => {
  ...
  (async () => {
    let windowId,tabId,debuggerAttached=false; const dashboardTabId=sender.tab?.id;
    try {
      ...
      const win = await chrome.windows.create({url:'about:blank',incognito:true,focused:false,width:job.device==='mobile'?430:1200,height:job.device==='mobile'?900:800}); windowId=win.id; tabId=win.tabs[0].id;
      ...
      const profile=await applyDeviceProfile(tabId,job.device);debuggerAttached=true;
      await notify(dashboardTabId,{type:'SEO_RANK_PROGRESS',id:job.id,progress:7,message:'پروفایل دستگاه و User-Agent اعمال شد.',debug:`USER_AGENT_APPLIED device=${job.device} profile=${profile.name} ua=${JSON.stringify(profile.userAgent)}`});
      await navigate(tabId,first);
    ...
    finally { if(debuggerAttached&&tabId)await chrome.debugger.detach({tabId}).catch(()=>{});if (windowId) chrome.windows.remove(windowId).catch(()=>{}); }
  })(); reply({accepted:true,protocol:7,version:chrome.runtime.getManifest().version}); return true;
});
