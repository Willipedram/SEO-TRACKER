...
`chrome://extensions`. The dashboard checks protocol version 7 and stops with an
explicit upgrade message instead of silently running a cached worker. A current run
logs `EXTENSION_VERSION | version=1.6.0 protocol=7 required=7`; older logs without
...
Before Google is loaded, the extension attaches Chrome DevTools Protocol to the temporary Incognito tab and applies a matching User-Agent and viewport: Windows desktop for `desktop`, and Android/Pixel mobile for `mobile`. Chromium therefore shows a debugger permission warning while a rank job is running; the debugger is detached and the Incognito window is closed when that job finishes. Batch runs create a fresh Incognito window and apply the correct profile for every keyword/device row.
