window.addEventListener('message', event => {
  ...
  chrome.runtime.sendMessage(event.data).then(response => {
    ...
    else if (response?.accepted) window.postMessage({source:'seo-tracker-extension',type:'SEO_RANK_ACK',id:event.data.id,protocol:response.protocol,version:response.version,message:'Extension worker accepted the rank job.'}, location.origin);
