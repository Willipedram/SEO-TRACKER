...
Phase 11 adds dashboard query and SVG presentation services inside `RankTracking`;
it does not create a separate module or couple Core to charting. Locale catalogs are
presentation resources, while rank calculations operate on provider-neutral accepted
results.

Phase 12 adds `SearchConsole` as an explicitly optional source module. The loader
isolates missing source and provider-registration failures, while its database-backed
feature flag defaults disabled and preserves all rows when toggled. Only an authorized
management shell registers in this phase; OAuth and sync capabilities remain absent.
Core and other modules do not reference its namespace or tables. See
`SEARCH_CONSOLE.md` and ADR 0014.

Phase 13 implements only the optional module's OAuth connection capability. Its Google
gateway, state store, authenticated token vault, property candidates/mapping, and
website-scoped controller remain inside `App\Modules\SearchConsole`. Websites exposes
no Search Console class dependency; Core and RankTracking remain unchanged. See ADR
0015.

Phase 14 keeps the queue, sync manager/worker, Search Analytics adapter call, lifecycle
logs, and imported rows wholly inside `SearchConsole`. Its CLI entry constructs the
optional factory only when explicitly invoked. Core and RankTracking do not query these
tables or require Google availability. See `SEARCH_CONSOLE_SYNC.md` and ADR 0016.
