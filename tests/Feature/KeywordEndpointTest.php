...
final class KeywordEndpointTest extends TestCase
{
    public function testKeywordListAndDirectPermissionDenial(): void
    {
        ...
        $this->assertTrue(!str_contains($response->body, '<script>safe</script>'));
        $this->assertTrue(str_contains($response->body, 'id="keyword-create-modal"'));
        $this->assertTrue(str_contains($response->body, 'name="keywords"'));
        $this->assertTrue(str_contains($response->body, 'name="devices[]" value="desktop" checked'));
        ...
        $this->assertTrue(str_contains($rankDashboard->body, 'manual-rank-start-all'));
