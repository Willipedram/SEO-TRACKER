...
final class KeywordEndpointTest extends TestCase
{
    public function testKeywordListAndDirectPermissionDenial(): void
    {
        ...
        $this->assertTrue(str_contains($rankDashboard->body, 'ردیابی دستی فعال است.'));
        $this->assertTrue(str_contains($rankDashboard->body, 'action="/rank-checks/manual"'));
        $this->assertTrue(str_contains($rankDashboard->body, 'https://www.google.com/search?'));
