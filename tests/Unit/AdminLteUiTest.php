...
final class AdminLteUiTest extends TestCase
{
    ...
    public function testApplicationStylesCoverResponsiveRtlAndReducedMotion(): void
    {
        ...
        foreach (['font-family:IRANSans','local("IRANSansX")','color-scheme:light','html[dir=rtl]','html,body{max-width:100%;overflow-x:hidden;color','width:min(calc(100% - 2rem),560px)','.app-main.card{width:auto;max-width:none!important','.app-header .container-fluid{display:flex;width:100%','.btn,.button{display:inline-flex','.footer-inner{display:flex;width:100%','margin-left:var(--lte-sidebar-width,250px)','html[dir=rtl] .app-header,html[dir=rtl] .app-footer{margin-left:0;margin-right:var(--lte-sidebar-width,250px)','body>.progress,body>#nprogress,body>.pace{display:none!important}','transform:translateX(-100%)','html[dir=rtl] .app-sidebar{transform:translateX(100%)','.logbox-filters','.logbox-filter.is-active','.level-warning','.logbox-filter-status','.logbox-output','.seo-table-responsive','--bs-table-bg:#fff','.rank-job-wait','@media(max-width:991.98px)','@media(max-width:575.98px)','prefers-reduced-motion','.rank-chart','.table-scroll','.installer-choice-grid'] as $required) $this->assertTrue(str_contains($css,$required));
    }
    ...
    public function testUserIpRankRunnerHasModalAndIncognitoExtensionContracts(): void
    {
        ...
        foreach(['SEO_RANK_START','SEO_RANK_ACK','SEO_RANK_PROGRESS','SEO_RANK_DONE','/rank-checks/manual','crypto.randomUUID()','PAGE_START','SAVE_RESPONSE','data-manual-rank-debug','lastProgress','progress = null','requiredProtocol = 4','EXTENSION_VERSION','chrome://extensions'] as $required)$this->assertTrue(str_contains($javascript,$required));
