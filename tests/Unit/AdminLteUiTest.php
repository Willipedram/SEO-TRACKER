...
final class AdminLteUiTest extends TestCase
{
    ...
    public function testApplicationStylesCoverResponsiveRtlAndReducedMotion(): void
    {
        ...
        foreach (['font-family:IRANSans','local("IRANSansX")','html[dir=rtl]','@media(max-width:991.98px)','@media(max-width:575.98px)','prefers-reduced-motion','.rank-chart','.table-scroll','.installer-choice-grid'] as $required) $this->assertTrue(str_contains($css,$required));
    }
    ...
    public function testCdnAndInlineAssetsHaveStrictCspCoverage(): void
    {
        ...
        $policy = SecurityHeaders::apply(Response::html($html), 'phase27-csp-test')->headers['Content-Security-Policy'];
        foreach (["script-src 'self' https://cdn.jsdelivr.net", "style-src 'self' https://cdn.jsdelivr.net", "font-src 'self' https://cdn.jsdelivr.net", "'sha256-"] as $required) $this->assertTrue(str_contains($policy,$required));
