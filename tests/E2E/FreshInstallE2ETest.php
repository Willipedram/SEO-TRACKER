...
final class FreshInstallE2ETest extends TestCase
{
    public function testExtractedReleaseCanCreateACompleteFreshApplicationState(): void
    {
        ...
        try {
            ...
            $this->withServer($extracted, function (string $origin) use ($extracted): void {
                ...
                $this->assertTrue(str_contains($installer['body'], 'Environment check'));
                $this->assertTrue(str_contains($installer['body'], 'Continue to database'));
                ...
                $this->assertTrue(str_contains($failedDatabase['body'], 'Could not connect to the database'));
    ...
    private function exerciseInstalledHttpFlows(string $origin): void
    {
        ...
        $this->assertTrue(str_contains($rank['body'], 'No approved Rank Tracking adapter is configured'));
...
$environment = [
    ...
    'APP_LOCALE="en"', 'APP_TRUSTED_HOSTS="localhost,127.0.0.1"', 'LOG_LEVEL="debug"',
