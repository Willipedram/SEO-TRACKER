...
final class AdminLayout
{
    ...
    private function sidebar(string $path, array $permissions, array $modules, string $baseUrl): string
    {
        $items = [
            ...
            ['/websites/search-console','bi-google','سرچ کنسول گوگل','search_console.connect','search_console'],
    ...
    private function components(DOMDocument $document): void
    {
        ...
        foreach (iterator_to_array($document->getElementsByTagName('td')) as $cell) {
            if (!$cell instanceof DOMElement || $cell->nextSibling !== null) continue;
            foreach (iterator_to_array($cell->getElementsByTagName('a')) as $link) if ($link instanceof DOMElement) $link->setAttribute('class', trim($link->getAttribute('class').' btn btn-sm btn-outline-primary'));
        }
