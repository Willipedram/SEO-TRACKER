...
final class SearchConsoleController
{
    ...
    public function status(): Response
    {
        try {
            ...
            $content .= '<div class="d-flex flex-wrap gap-2 mb-4"><form method="post" action="/admin/modules/search-console/status"><input type="hidden" name="_token" value="' . Html::escape(Csrf::token()) . '"><input type="hidden" name="enabled" value="' . ($model['enabled'] ? '0' : '1') . '"><button class="btn btn-' . ($model['enabled'] ? 'outline-danger' : 'primary') . '">' . Html::escape($t->get($model['enabled'] ? 'disable' : 'enable')) . '</button></form></div>';
            $content .= '<section class="card border-0 bg-body-tertiary mb-4"><div class="card-body"><h2 class="h5">' . Html::escape($t->get('connection_steps')) . '</h2><ol class="mb-0"><li>' . Html::escape($t->get('step_create_oauth')) . '</li><li>' . Html::escape($t->get('step_redirect')) . ' <code dir="ltr">' . Html::escape((string) ($model['redirect_uri'] ?? '—')) . '</code></li><li>' . Html::escape($t->get('step_connect')) . '</li><li>' . Html::escape($t->get('step_sync_worker')) . ' <code>php bin/console search-console:work --limit=10</code></li></ol></div></section>';
            if ($model['enabled']) { $content .= '<div class="row g-3">'; foreach ($model['websites'] as $website) $content .= '<div class="col-12 col-md-6 col-xl-4"><div class="card h-100 shadow-sm border-0"><div class="card-body d-flex align-items-center justify-content-between gap-3"><div><h3 class="h6 mb-1">' . Html::escape((string) $website['site_name']) . '</h3><small class="text-body-secondary">' . Html::escape($t->get('website_title')) . '</small></div><a class="btn btn-primary btn-sm" href="/websites/search-console?website=' . rawurlencode((string) $website['public_id']) . '"><i class="bi bi-google" aria-hidden="true"></i> ' . Html::escape($t->get('manage_connection')) . '</a></div></div></div>'; $content .= '</div>'; }
    ...
    public function website(Request $request): Response
    {
        ...
        try {
            ...
            if (!is_string($request->query['website'] ?? null) || $request->query['website'] === '') {
                $cards = '';
                foreach ($connections->websites($actor) as $item) $cards .= '<div class="col-12 col-md-6 col-xl-4"><div class="card h-100 border-0 shadow-sm"><div class="card-body"><h2 class="h5">' . Html::escape((string)$item['site_name']) . '</h2><p class="text-body-secondary technical-ltr">' . Html::escape((string)$item['normalized_domain']) . '</p><a class="btn btn-primary" href="/websites/search-console?website=' . rawurlencode((string)$item['public_id']) . '"><i class="bi bi-google" aria-hidden="true"></i> ' . Html::escape($t->get('manage_connection')) . '</a></div></div></div>';
                $content = $cards === '' ? '<div class="alert alert-info">' . Html::escape($t->get('no_websites')) . '</div>' : '<p class="text-body-secondary">' . Html::escape($t->get('choose_website')) . '</p><div class="row g-3">' . $cards . '</div>';
            ...
            return $this->page($t->get('website_title'), $content);
        } catch (SearchConsoleUnavailable $exception) { return $this->page($t->get('website_title'), '<div class="alert alert-warning"><strong>' . Html::escape($t->get($exception->getMessage())) . '</strong><p class="mb-0">' . Html::escape($t->get('configure_module_help')) . '</p></div><a class="btn btn-primary" href="/admin/modules/search-console">' . Html::escape($t->get('configure_module')) . '</a>', 422); }
