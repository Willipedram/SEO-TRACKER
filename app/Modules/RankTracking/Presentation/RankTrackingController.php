    ...
    public function dashboard(Request $request): Response
    {
        ...
        try {
            ...
            foreach ($model['rows'] as $row) {
                ...
                $manual = '<button class="btn btn-sm btn-primary manual-rank-start" type="button" data-website="' . Html::escape($website) . '" data-keyword-id="' . Html::escape((string)$row['public_id']) . '" data-query="' . Html::escape((string)$row['keyword_text']) . '" data-domain="' . Html::escape((string)$model['website']['normalized_domain']) . '" data-country="' . Html::escape(strtolower((string)$row['country_code'])) . '" data-language="' . Html::escape(strtolower((string)$row['language_code'])) . '" data-device="' . Html::escape((string)$row['device']) . '"' . ((int)$row['active'] === 1 ? '' : ' disabled') . '>رتبه‌یابی با IP من</button>';
            ...
            $runAll = $model['rows'] === [] ? '' : '<div class="d-flex justify-content-end mb-3"><button class="btn btn-primary manual-rank-start-all" type="button"><i class="bi bi-play-circle" aria-hidden="true"></i> رتبه‌یابی همه کلیدواژه‌ها با IP من</button></div>';
            ...
            return $this->localizedPage($translator->get('dashboard') . ' — ' . $model['website']['site_name'], $configuration . $filters . $runAll . $empty . $table . $modal, $translator);
